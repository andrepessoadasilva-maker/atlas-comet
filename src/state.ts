...
export class AppState {
   ...
   * Returns null for the special "/a/tickets/new" path to avoid collision with
   * the new ticket form route.
   *
  ...
  public extractTicketIdFromUrl(url: string): string | null {
    ...
    // Exclude the /a/tickets/new path so it doesn't interfere with the new ticket observer
    if (url.includes(CONSTANTS.URL.NEW_TICKET_PATH)) return null;
    const match = url.match(new RegExp(`${CONSTANTS.URL.TICKETS_PATH}(\\d+)`));
    return match ? match[1] : null;
  }

  /**
   * Checks if the current URL corresponds to the "New Ticket" form page.
   *
   * Matches paths like:
   * - https://cvcrm.freshdesk.com/a/tickets/new
   * - https://anapro.freshdesk.com/a/tickets/new
   *
   * @param url - The full URL string to check.
   * @returns {boolean} True if the URL is the new ticket page.
   */
  public isNewTicketUrl(url: string): boolean {
    return url.includes(CONSTANTS.URL.NEW_TICKET_PATH);
  }
}
